<?php
/**
 * Contoh Script Bridge API Rapidtv via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Rapidtv di sistem Bridge
function callRapidtvAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/rapidtv/api/v1";

// Contoh 1: Fitur Cari Film Rapidtv
echo "\n=====================[ 1. Pencarian Rapidtv ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callRapidtvAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callRapidtvAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callRapidtvAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/rapidtv ◀◀◀\n\n";
